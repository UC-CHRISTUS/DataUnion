...
import { createMockSigesa } from '../utils/fixtures';
...
describe('GET /api/v1/sigesa', () => {
  beforeEach(() => {
    const sigesa = [
      createMockSigesa({ id: 1, created_at: '2025-01-15T10:00:00Z' }),
      createMockSigesa({ id: 2, created_at: '2025-01-16T10:00:00Z' }),
    ];
  ...
  describe('Success Cases - Default Pagination', () => {
    ...
    it('should return SIGESA documents ordered by id descending', async () => {
      // Arrange
      const sigesaDocs = [
        createMockSigesa({ id: 3, created_at: '2025-01-17T10:00:00Z' }),
        createMockSigesa({ id: 2, created_at: '2025-01-16T10:00:00Z' }),
        createMockSigesa({ id: 1, created_at: '2025-01-15T10:00:00Z' }),
      ];
      mockSupabaseInstance = createMockSupabaseClient({ sigesa: sigesaDocs });
  ...
  describe('Edge Cases', () => {
    it('should return empty data array when no SIGESA documents exist', async () => {
      // Arrange
      mockSupabaseInstance = createMockSupabaseClient({ sigesa: [] });
      const request = createMockNextRequest({
        method: 'GET',
        url: 'http://localhost:3000/api/v1/sigesa',
      });
  ...
  describe('Pagination Calculation', () => {
    it('should calculate totalPages correctly', async () => {
      ...
      const manyRecords = Array.from({ length: 25 }, (_, i) =>
        createMockSigesa({ id: i + 1, created_at: new Date().toISOString() })
      );
      mockSupabaseInstance = createMockSupabaseClient({ sigesa: manyRecords });
